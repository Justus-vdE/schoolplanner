    ...
    <div class="plan-task ${isDone ? 'done' : ''}">
      <div class="plan-task-top">
        ...
        <div class="plan-task-actions">
          ${!isDone ? `<button class="btn btn-outline btn-sm plan-log-btn" onclick="openFocusStartModal(${plan.id},${t.id})" title="Start studeersessie">&#9654;&#65039;</button>` : ''}
...
// ==================== STUDIE-EILAND & FOCUS-SESSIES ====================
function renderIslandScene(size) {
  const unlocked = unlockedIslandItems(gameStats.xp);
  return `
    <div class="island-scene ${size === 'big' ? 'big' : ''}">
      <div class="island-sea"></div>
      <div class="island-land"></div>
      ${unlocked.map((it, i) => {
        const spot = islandSpots[i % islandSpots.length];
        return `<span class="island-item" style="left:${spot.x}%;top:${spot.y}%" title="${it.name}">${it.emoji}</span>`;
      }).join('')}
      ${unlocked.length === 0 ? '<span class="island-empty">Nog een leeg eiland...<br>Start een studeersessie!</span>' : ''}
    </div>
  `;
}
...
function renderGameCard() {
  loadGame();
  const lvl = gameLevel(gameStats.xp);
  const next = nextIslandItem(gameStats.xp);
  const hours = Math.floor(gameStats.minutes / 60);
  const mins = gameStats.minutes % 60;

  return `
    <div class="card">
      <div class="card-header">
        <div class="card-title">&#127965;&#65039; Jouw studie-eiland</div>
        <span style="font-size:0.8rem;color:var(--gray-400)">${lvl.icon} ${lvl.name}</span>
      </div>
      ${renderIslandScene()}
      ${next ? `
        <div class="island-next">
          Nog <strong>${next.xp - gameStats.xp} XP</strong> (&plusmn;${next.xp - gameStats.xp} min leren) tot ${next.emoji} ${next.name}
          <div class="island-next-bar"><div class="island-next-fill" style="width:${Math.round((1 - (next.xp - gameStats.xp) / (next.xp - (islandItems[islandItems.indexOf(next) - 1] ? islandItems[islandItems.indexOf(next) - 1].xp : 0))) * 100)}%"></div></div>
        </div>
      ` : '<div class="island-next">&#127881; Je eiland is compleet — wat een legende!</div>'}
      <div class="island-stats">
        <span>&#9889; ${gameStats.xp} XP</span>
        <span>&#9201;&#65039; ${hours}u ${mins}m geleerd</span>
        <span>&#127942; ${gameStats.badges.length}/${gameBadges.length}</span>
      </div>
      <button class="btn btn-primary" style="width:100%;justify-content:center;margin-top:10px" onclick="openFocusStartModal()">
        &#9654;&#65039; Start studeersessie
      </button>
    </div>
  `;
}

// --- Focus-sessie starten ---
let focusSession = null;
let focusInterval = null;

function openFocusStartModal(presetPlanId, presetTaskId) {
  loadPlans();
  const plan = getDashboardPlan();
  const tasks = plan ? plan.tasks.filter(t => !(t.hoursDone >= t.hours)) : [];

  openModal('Studeersessie starten', `
    <p style="color:var(--gray-500);font-size:0.85rem;margin:0 0 14px">
      Kies waar je aan gaat werken en hoelang. Elke geleerde minuut = 1 XP voor je eiland &mdash; en je uren worden automatisch in je planning gelogd. Sessie helemaal afmaken = +25% bonus!
    </p>
    <div class="form-group">
      <label class="form-label">Taak</label>
      <select class="form-select" id="focus-task">
        <option value="">Vrij leren (zonder taak)</option>
        ${tasks.map(t => {
          const s = subjects[t.subject];
          const sel = plan && presetPlanId === plan.id && presetTaskId === t.id ? 'selected' : '';
          return `<option value="${plan.id}:${t.id}" ${sel}>${s ? s.name : ''} — ${esc(t.title)}</option>`;
        }).join('')}
      </select>
    </div>
    <div class="form-group">
      <label class="form-label">Hoelang?</label>
      <div class="focus-durations">
        ${[15, 25, 45, 60].map(m => `<button type="button" class="focus-dur ${m === 25 ? 'active' : ''}" data-min="${m}" onclick="document.querySelectorAll('.focus-dur').forEach(b=>b.classList.remove('active'));this.classList.add('active')">${m} min</button>`).join('')}
      </div>
    </div>
    <button class="btn btn-primary" style="width:100%;justify-content:center" onclick="startFocusSession()">&#9654;&#65039; Start</button>
  `);
}

function startFocusSession() {
  const taskVal = document.getElementById('focus-task').value;
  const durBtn = document.querySelector('.focus-dur.active');
  const minutes = durBtn ? parseInt(durBtn.dataset.min) : 25;
  let planId = null, taskId = null, label = 'Vrij leren';
  if (taskVal) {
    [planId, taskId] = taskVal.split(':').map(Number);
    const p = getPlan(planId);
    const t = p && p.tasks.find(x => x.id === taskId);
    if (t) label = t.title;
  }
  ...
  focusInterval = setInterval(tickFocus, 500);
  tickFocus();
}

function focusRemainingMs() {
  if (!focusSession) return 0;
  return focusSession.paused ? focusSession.remainingMs : Math.max(0, focusSession.endTime - Date.now());
}

function tickFocus() {
  if (!focusSession) return;
  const remaining = focusRemainingMs();
  if (remaining <= 0 && !focusSession.paused) { stopFocus(true); return; }
  const el = document.getElementById('focus-overlay');
  if (!el) return;
  const totalMs = focusSession.totalMin * 60000;
  ...
  el.innerHTML = `
    <div class="focus-card">
      <div class="focus-label">${esc(focusSession.label)}</div>
      <div class="focus-ring">
        <svg width="220" height="220">
          <circle cx="110" cy="110" r="${r}" fill="none" stroke="rgba(255,255,255,0.15)" stroke-width="10"/>
          <circle cx="110" cy="110" r="${r}" fill="none" stroke="#22C55E" stroke-width="10" stroke-linecap="round"
            stroke-dasharray="${c}" stroke-dashoffset="${c * (1 - pct)}" transform="rotate(-90 110 110)"/>
        </svg>
        <div class="focus-time">${mm}:${String(ss).padStart(2, '0')}</div>
      </div>
      <p class="focus-hint">${focusSession.paused ? '&#9208;&#65039; Gepauzeerd' : 'Blijf erbij — elke minuut groeit je eiland &#127965;&#65039;'}</p>
      <div class="focus-actions">
        <button class="btn btn-outline" onclick="togglePauseFocus()">${focusSession.paused ? '&#9654;&#65039; Hervat' : '&#9208;&#65039; Pauze'}</button>
        <button class="btn btn-primary" onclick="stopFocus(false)">&#9209;&#65039; Stop &amp; log</button>
        <button class="btn btn-outline focus-cancel" onclick="cancelFocus()">Afbreken</button>
      </div>
    </div>
  `;
}
...
function togglePauseFocus() {
  ...
  if (focusSession.paused) {
    focusSession.endTime = Date.now() + focusSession.remainingMs;
    focusSession.paused = false;
    focusSession.remainingMs = null;
  } else {
    focusSession.remainingMs = focusRemainingMs();
...
function cancelFocus() {
  if (!confirm('Sessie afbreken zonder iets te loggen?')) return;
  closeFocusOverlay();
}
...
function stopFocus(completed) {
  if (!focusSession) return;
  const elapsedMs = focusSession.totalMin * 60000 - focusRemainingMs();
  const minutes = Math.max(completed ? 1 : 0, Math.round(elapsedMs / 60000));
  if (focusInterval) { clearInterval(focusInterval); focusInterval = null; }

  if (minutes < 1) { closeFocusOverlay(); return; }

  // Uren loggen op de gekozen taak
  let taskDoneNow = false;
  if (focusSession.planId != null && focusSession.taskId != null) {
    const p = getPlan(focusSession.planId);
    const t = p && p.tasks.find(x => x.id === focusSession.taskId);
    if (t) {
      const wasDone = t.hoursDone >= t.hours;
      t.hoursDone = Math.min(t.hours, Math.round((Math.max(0, t.hoursDone || 0) + minutes / 60) * 100) / 100);
      t.done = t.hoursDone >= t.hours;
      if (t.done && !wasDone) { taskDoneNow = true; gameStats.tasksCompleted = (gameStats.tasksCompleted || 0) + 1; }
      savePlans();
    }
  }
  updateStreak();
  const result = awardSession(minutes, !!completed);
  ...
  if (el) {
    el.innerHTML = `
      <div class="focus-card">
        <div style="font-size:3rem">${completed ? '🎉' : '👍'}</div>
        <h2 style="color:#fff;margin:8px 0 4px">${completed ? 'Sessie voltooid!' : 'Goed bezig!'}</h2>
        <p class="focus-hint">${minutes} minuten geleerd${focusSession.label !== 'Vrij leren' ? ` aan "${esc(focusSession.label)}"` : ''}</p>
        <div class="focus-rewards">
          <div class="focus-reward">+${result.xp} XP ${result.bonus ? `<span class="focus-bonus">(+${result.bonus} bonus)</span>` : ''}</div>
          ${taskDoneNow ? '<div class="focus-reward">✅ Taak helemaal af!</div>' : ''}
          ${result.newItems.map(it => `<div class="focus-reward new-item">${it.emoji} Nieuw op je eiland: <strong>${it.name}</strong></div>`).join('')}
          ${result.leveledUp ? `<div class="focus-reward">⬆️ Level omhoog: ${result.level.icon} <strong>${result.level.name}</strong></div>` : ''}
          ${result.newBadges.map(b => `<div class="focus-reward">${b.icon} Badge: <strong>${b.name}</strong></div>`).join('')}
        </div>
        <button class="btn btn-primary" style="margin-top:18px;padding:12px 32px" onclick="finishFocusCelebration()">Verder</button>
      </div>
    `;
  }
  focusSession = null;
...
function finishFocusCelebration() {
  closeFocusOverlay();
  renderPage(currentPage);
}
